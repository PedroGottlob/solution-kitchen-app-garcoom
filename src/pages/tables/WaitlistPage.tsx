    ...
    <div className="min-h-screen bg-zinc-50 flex flex-col pb-28">
      ...
      <div className="bg-accent-50 border-b border-accent-200 px-5 py-4 flex items-center gap-3">
        ...
        <div className="flex-1">
          <h1 className="text-zinc-900 text-xl font-medium">Fila de espera</h1>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 gap-3">
          <i className="ti ti-users text-zinc-500 text-4xl" />
          <p className="text-zinc-500">Ninguém na fila de espera</p>
          <p className="text-zinc-500 text-xs">Toque no botão + pra cadastrar um cliente</p>
        </div>
      ) : (
        ...
        <div className="fixed inset-0 bg-black/60 flex items-end z-[60]" onClick={() => setShowForm(false)}>
          <div className="bg-accent-50 rounded-t-2xl w-full p-5 pb-8 flex flex-col gap-4 max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex flex-col gap-1">
              ...
              <input type="text" value={form.customerName} onChange={e => setForm(f => ({ ...f, customerName: e.target.value }))} placeholder="Ex: João" className="bg-zinc-200 text-zinc-900 text-sm rounded-lg px-3 py-2.5 outline-none border border-zinc-300" />
            </div>

            <div className="flex flex-col gap-1">
              <label className="text-zinc-500 text-xs">Quantidade de pessoas</label>
              <div className="flex items-center gap-2">
                <button onClick={() => adjustPartySize(-1)} className="w-9 h-9 rounded-lg bg-zinc-200 flex items-center justify-center cursor-pointer hover:bg-zinc-300 transition-colors border border-zinc-300">
                  <i className="ti ti-minus text-zinc-600 text-sm" />
                </button>
                <input type="number" min="1" value={form.partySize} onChange={e => setForm(f => ({ ...f, partySize: e.target.value }))} placeholder="2" className="flex-1 bg-zinc-200 text-zinc-900 text-sm rounded-lg px-3 py-2.5 outline-none border border-zinc-300 text-center" />
                <button onClick={() => adjustPartySize(1)} className="w-9 h-9 rounded-lg bg-zinc-200 flex items-center justify-center cursor-pointer hover:bg-zinc-300 transition-colors border border-zinc-300">
