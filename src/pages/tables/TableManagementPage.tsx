    ...
    <div className="min-h-screen bg-zinc-50 flex flex-col pb-28">
      ...
      <div className="bg-accent-50 border-b border-accent-200 px-5 py-4 flex items-center gap-3">
        ...
        <div className="flex-1">
          <h1 className="text-zinc-900 text-xl font-medium">Gestão de mesas</h1>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 gap-3">
          <i className="ti ti-armchair text-zinc-500 text-4xl" />
          <p className="text-zinc-500">Nenhuma mesa cadastrada</p>
          <p className="text-zinc-500 text-xs">Toque no botão + para adicionar a primeira</p>
        </div>
      ) : (
        ...
        <div className="fixed inset-0 bg-black/60 flex items-end z-[60]" onClick={() => setShowForm(false)}>
          <div className="bg-accent-50 rounded-t-2xl w-full p-5 pb-8 flex flex-col gap-4 max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex flex-col gap-1">
              ...
              <input type="number" min="1" value={form.number} onChange={e => setForm(f => ({ ...f, number: e.target.value }))} placeholder="Ex: 13" disabled={!!editingId} className="bg-zinc-200 text-zinc-900 text-sm rounded-lg px-3 py-2.5 outline-none border border-zinc-300 disabled:opacity-60 disabled:cursor-not-allowed" />
              {editingId && <p className="text-zinc-500 text-xs italic">Número não pode ser alterado</p>}
            </div>

            <div className="flex flex-col gap-1">
              <label className="text-zinc-500 text-xs">Nome (opcional)</label>
              <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Ex: Varanda" className="bg-zinc-200 text-zinc-900 text-sm rounded-lg px-3 py-2.5 outline-none border border-zinc-300" />
            </div>

            <div className="flex flex-col gap-1">
              <label className="text-zinc-500 text-xs">Capacidade (pessoas)</label>
              <div className="flex items-center gap-2">
                <button onClick={() => adjustCapacity(-1)} className="w-9 h-9 rounded-lg bg-zinc-200 flex items-center justify-center cursor-pointer hover:bg-zinc-300 transition-colors border border-zinc-300">
                  <i className="ti ti-minus text-zinc-600 text-sm" />
                </button>
                <input type="number" min="1" value={form.capacity} onChange={e => setForm(f => ({ ...f, capacity: e.target.value }))} placeholder="4" className="flex-1 bg-zinc-200 text-zinc-900 text-sm rounded-lg px-3 py-2.5 outline-none border border-zinc-300 text-center" />
                <button onClick={() => adjustCapacity(1)} className="w-9 h-9 rounded-lg bg-zinc-200 flex items-center justify-center cursor-pointer hover:bg-zinc-300 transition-colors border border-zinc-300">
                  <i className="ti ti-plus text-zinc-600 text-sm" />
                </button>
              </div>
            </div>
